...
# p and q for starts_combined_dataset_above_90 TODO: replace with your datasets statistics
p = 0.086
q = 0.914
p_f2 = 0.045
p_f3 = 0.041
...
def get_region_probability(starts, start, stop, gene="bla"):
	...
	# the upper limit on the probability
	total_prob = 0	
...
outFile = open(sys.argv[1] + "_with_found_frameshifts", 'w')
...
while gene:
	...
	for i in range(0, len(ss) - 3, 3): # i is in frame
		...
		if ((before_frameshift_prob != "UNDEF") and (frameshift_prob != "UNDEF") and ((stop - start) > 30)):
			total_prob = min(0, (frameshift_prob - before_frameshift_prob))
			# don't consider situtations where 2 and 3 counts are not greater than average, and where before counts are greater than after counts
			# also where there are more reads in a the wrong frame
